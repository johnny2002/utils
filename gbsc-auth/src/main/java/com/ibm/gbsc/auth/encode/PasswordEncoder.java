...
/**
 * 本工具类用于生成临时密码.
 * 使用方法为：PasswordEncoder.encode("passowrd");
 * 
 * @author Johnny
 * 
 */
public class PasswordEncoder {
	// private final int[] salt = { 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476, 0xc3d2e1f0 };
	// 摘要数据存储数组
	private final int[] digestInt = new int[5];
	// 计算过程中的临时数据存储数组
	private final int[] tmpData = new int[80];

	private int processInputBytes(byte[] bytedata, int[] salt) {
		// int[] salt = { 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476, 0xc3d2e1f0 };// saltString.toCharArray();
		// 初试化常量
		System.arraycopy(salt, 0, digestInt, 0, salt.length);
		// 格式化输入字节数组，补10及长度数据
	...
	// 计算摘要，返回相应的字节数组
	byte[] getDigestOfBytes(byte[] byteData, int[] salt) {
		processInputBytes(byteData, salt);
	 ...
	 * @return 编码后的密码串
	 */
	public static String encode(String password, int[] salt) {

		return byteArrayToHexString(new PasswordEncoder().getDigestOfBytes(password.getBytes(), salt));
	}
}